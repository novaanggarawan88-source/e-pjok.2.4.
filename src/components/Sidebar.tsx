...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar container - Terkunci Permanen & Tidak Geser Naik Turun */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-40 h-full w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-transform duration-300 ease-in-out shrink-0 select-none ${
          isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        {/* Mobile header with close button */}
        <div className="flex items-center justify-between p-3.5 pb-3 border-b border-slate-100 dark:border-slate-800 lg:hidden shrink-0">
        ...
        {/* Scrollable Navigation Groups (Hanya daftar menu yang scroll jika layar pendek) */}
        ...
        {/* Bottom Section: Tombol Keluar Terkunci Permanen di Paling Bawah */}
        <div className="p-3.5 border-t border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
          <button
            type="button"
            onClick={handleLogout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2.5 text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/40 border border-rose-200 dark:border-rose-900/50 rounded-xl transition-all cursor-pointer shadow-xs active:scale-[0.99]"
          >
